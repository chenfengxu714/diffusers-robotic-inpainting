...
class ImageProcessor:
    def __init__(self, base_model_path, controlnet_path, target_robot, device="cuda:0", batch_size=128):
        self.base_model_path = base_model_path
        self.controlnet_path = controlnet_path
        self.batch_size = batch_size
        self.prompt = f"create a high quality image with a {target_robot} robot and white background"
        self.to_tensor = transforms.ToTensor()
        self.to_pil = transforms.ToPILImage()
        
        self.controlnet = ControlNetModel.from_pretrained(controlnet_path, torch_dtype=torch.float16).to(device=device)
    ...
    def process_folders(self, image_trajectory): # image_trajectory is M x H x W x 3
        ...
        list_of_input_images = [self.to_pil(image_trajectory[i]) for i in range(image_trajectory.shape[0])]
        ...
        generated_images = [np.array(self.to_pil(self.to_tensor(generated_image).clamp(0, 1))) for generated_image in generated_images]
        ...
        # Image.fromarray(generated_images[0]).save("test.png")
